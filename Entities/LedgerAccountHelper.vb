...
Public Class LedgerAccountHelper
    ...
    Public Function AddLedgerAccount(ByVal account As Accounts) As Boolean
        ...
        Try
            ...
            saveQuery = "if ((select count(*) from CG_Accounts where AM_Acc_Cd=@AccCode and AM_Inst_Cd=@InstitutionCode and AM_Inst_Typ=@InstType and AM_Fin_Yr=@finYear)=0)" + _
                " Begin " + _
                "INSERT INTO CG_Accounts ([AM_Fin_Yr] ,[AM_Inst_Cd],[AM_Inst_Typ],[AM_Brn_Cd],[AM_Lgr_Cd],[AM_Acc_Cd],[AM_Acc_Nm],[AM_Opn_Bal],[AM_OB_Cr_Dr],[AM_ABS_Opn_Bal],[AM_LLY_Budg],[AM_LLY_Actu],[AM_LYr_Budg],[AM_LYr_Actu],[AM_Cyr_Budg],[AM_Ent_By],[AM_Ent_Dt])" + _
            ...
            parameters.Add("@createBy", "CG")
            parameters.Add("@createdOn", DateTime.Now)
            parameters.Add("@updatedBy", "CG")
    ...
    Function GetAccountDetails(pAccCode As String, Optional pNotLinked As Boolean = False) As DataTable
        ...
        Try
            ...
            If String.IsNullOrEmpty(pAccCode) Then
                ...
                ",[AM_Ent_Dt],[AM_Upd_By],[AM_Upd_Dt] FROM CG_Accounts where AM_Inst_Cd='{0}' and AM_Inst_Typ='{1}' and AM_Fin_Yr='{2}'", InstitutionMasterData.XInstCode, InstitutionMasterData.XInstType, InstitutionMasterData.XFinYr))
            Else
                query.Append(String.Format("SELECT [AM_Fin_Yr],[AM_Inst_Cd],[AM_Inst_Typ],[AM_Brn_Cd],[AM_Lgr_Cd],[AM_Acc_Cd],Ltrim(Rtrim([AM_Acc_Nm])) as AM_Acc_Nm, [AM_Acc_Cd] +'_'+Ltrim(Rtrim([AM_Acc_Nm])) as name,[AM_Calls],[AM_Opn_Bal],[AM_OB_Cr_Dr],[AM_ABS_Opn_Bal],[AM_LLY_Budg],[AM_LLY_Actu],[AM_LYr_Budg],[AM_LYr_Actu],[AM_Cyr_Budg],[AM_Ent_By]" + _
                     ",[AM_Ent_Dt],[AM_Upd_By],[AM_Upd_Dt] FROM CG_Accounts  where AM_Acc_Cd='{0}' and AM_Inst_Cd='{1}' and AM_Inst_Typ='{2}' and AM_Fin_Yr='{3}'", pAccCode, InstitutionMasterData.XInstCode, InstitutionMasterData.XInstType, InstitutionMasterData.XFinYr))
            End If
    ...
    Function FillAccountCode() As DataTable
        ...
        Try
            query = New StringBuilder()
            query.Append(String.Format("SELECT [AM_Acc_Cd],Ltrim(Rtrim([AM_Acc_Nm])) as AM_Acc_Nm, Ltrim(Rtrim([AM_Acc_Nm])) +'_'+[AM_Acc_Cd] as name  FROM CG_Accounts where AM_Inst_Cd='{0}' and AM_Inst_Typ='{1}' and AM_Fin_Yr='{2}' order by [AM_Acc_Nm] asc", InstitutionMasterData.XInstCode, InstitutionMasterData.XInstType, InstitutionMasterData.XFinYr))
            Return dataHelper.ExecuteQuery(query.ToString, CommandType.Text, Nothing)
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function GetAccDetails(ByVal AccCode As String) As DataTable
        ...
        Dim str As String
        str = String.Format("select [AM_ABS_Opn_Bal],[AM_LLY_Budg],[AM_LLY_Actu],[AM_LYr_Budg],[AM_LYr_Actu],[AM_Cyr_Budg],[AM_OB_Cr_Dr] FROM CG_Accounts where AM_Acc_Cd='{0}' ", AccCode)
    ...
    Function GetTransactionCount(ByVal pAccCode As String, ByVal pDayBookCode As String) As Integer
        ...
        Try
            ...
            query = String.Format("SELECT count(*) from CG_Voucher_Header where VH_Fin_Yr='{1}' and VH_Inst_Cd='{2}' and VH_Inst_Typ='{3}'" + _
    ...
    Sub DeleteAccount(ByVal pAccCode As String)
        Try
            dataHelper.ExecuteNonQuery(String.Format("Delete from CG_Accounts where AM_Acc_Cd='{0}' and AM_Inst_Cd='{1}' and AM_Inst_Typ='{2}' and AM_Fin_Yr={3}", pAccCode, InstitutionMasterData.XInstCode, InstitutionMasterData.XInstType, InstitutionMasterData.XFinYr), CommandType.Text)
        Catch ex As Exception
            Throw
        End Try
    End Sub
    ...
    Function IsAccountLinked(ByVal pAccountCode As String, ByVal pDaybookcode As String) As Boolean
        Dim count As Integer
        Dim query As String = String.Format("Select AM_Acc_Cd,AM_Calls from CG_Accounts where AM_Inst_Cd='{1}' and AM_Inst_Typ='{2}' " + _
    ...
    Function GetBalance(Optional ByVal pDate? As DateTime = Nothing, Optional ByVal pDaybookcode As String = "") As Double
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(pDaybookcode) Then
                ...
                query = "Select DM_Acc_Cd from  CG_Daybooks where DM_Dbk_Cd='" + pDaybookcode + "'"
            ...
            Else
                ...
                query = "Select DM_Acc_Cd from CG_Daybooks  where DM_Dbk_Typ='C'"
                dtAcc = dataHelper.ExecuteQuery(query, CommandType.Text, Nothing)
            End If
    ...
    Function CheckAccCd(ByVal str As String) As DataTable
        ...
        Try
            query = New StringBuilder()
            query.Append(String.Format("select [AM_Acc_Cd] from CG_Accounts where AM_Inst_Cd='{0}' and AM_Inst_Typ='{1}' and AM_Fin_Yr='{2}' and [AM_Acc_Cd] like  '%" & str & "%' ", InstitutionMasterData.XInstCode, InstitutionMasterData.XInstType, InstitutionMasterData.XFinYr))
            Return dataHelper.ExecuteQuery(query.ToString, CommandType.Text, Nothing)
        Catch ex As Exception
            Throw
        End Try
